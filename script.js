...
class EventCalendar {
    constructor(opts) {
        ...
        const now = new Date();
        // Set to July 2025 to match screenshot
        this.year = 2025;
        this.month = 6; // July (0-indexed)
    ...
    renderMonthList(events) {
        ...
        events.forEach(e => {
            ...
            item.innerHTML = `
                <span class="event-date-badge">${formatDateShort(this.year, this.month, e.day)}</span>
                <div class="event-details">
                    <h3>${e.name}</h3>
                    <p class="event-location">場所：${e.location}</p>
                    <p class="event-time">時間：${e.time}</p>
                </div>`;
    ...
    renderAllUpcoming() {
        ...
        list.forEach(e => {
            const li = document.createElement('li');
            li.innerHTML = `
                <span class="u-date">${e.year}/${formatDateShort(e.year, e.month, e.day)}</span>
                <span class="u-name">${e.name}</span>
                <span class="u-loc">${e.location}</span>`;
